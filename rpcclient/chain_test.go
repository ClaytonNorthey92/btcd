...
func makeClient(t *testing.T) (*Client, chan string, func()) {
	...
	s := httptest.NewServer(http.HandlerFunc(makeUpgradeOneConnect(serverReceivedChannel)))
...
func makeUpgradeOneConnect(ch chan string) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			...
			err = c.WriteMessage(mt, []byte("blahhhhhh"))
			if err != nil {
				break
			}
		}
